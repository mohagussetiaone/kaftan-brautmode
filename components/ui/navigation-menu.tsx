...
function NavigationMenu({
  ...
  children,
  viewport = true,
  fullWidth = false, // Tambahkan prop ini
  ...props
}: React.ComponentProps<typeof NavigationMenuPrimitive.Root> & {
  viewport?: boolean;
  fullWidth?: boolean; // Tambahkan ini
}) {
  return (
    <NavigationMenuPrimitive.Root
      data-slot="navigation-menu"
      data-viewport={viewport}
      data-fullwidth={fullWidth} // Tambahkan data attribute
...
function NavigationMenuViewport({
  ...
  return (
    <div className={cn("absolute top-full left-0 isolate z-50 flex justify-center", fullWidth && "w-full")}>
      <NavigationMenuPrimitive.Viewport
        data-slot="navigation-menu-viewport"
        className={cn(
          "origin-top-center bg-popover text-popover-foreground data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-90 relative mt-1.5 h-[var(--radix-navigation-menu-viewport-height)] w-full overflow-hidden rounded-md border shadow",
          fullWidth
            ? "md:w-[100vw] left-0 right-0" // Full width untuk desktop
            : "md:w-[var(--radix-navigation-menu-viewport-width)]", // Default
          className
        )}
        {...props}
      />
